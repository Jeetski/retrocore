// Retrocore managed CRT shader for Windows Terminal.
// Generated from retrocore.json.
...
float3 phosphor_dot_glow(float2 uv, float3 baseColor)
{
    ...
    return baseColor * dotGlow * __DOT_GLOW_AMOUNT__;
}

float4 main(PSInput input) : SV_TARGET
{
    float2 uv = input.uv;
    float2 warped = barrel(uv, __CURVATURE__);
    ...
    float2 aberration = float2(__ABERRATION_BASE__ + edgeAmount * __ABERRATION_EDGE__, 0.0);
    ...
    float noise = (rand(float2(warped.y * 241.13, Time * 2.7)) - 0.5) * __NOISE_AMOUNT__;
    float flicker = 1.0 + sin(Time * 59.7) * __FLICKER_PRIMARY__ + sin(Time * 121.3) * __FLICKER_SECONDARY__;
    float verticalJitter = (rand(float2(floor(Time * 2.0), 8.0)) - 0.5) * __VERTICAL_JITTER__;

    float3 persistence =
        shaderTexture.Sample(samplerState, warped + float2(0.0, verticalJitter + 1.0 / Resolution.y)).rgb * __PERSISTENCE_AMOUNT__ +
        shaderTexture.Sample(samplerState, warped + float2(0.0, verticalJitter - 1.0 / Resolution.y)).rgb * __PERSISTENCE_AMOUNT__;

    float3 color = base;
    color += glow * __GLOW_HORIZONTAL__;
    color += scanGlow * __GLOW_VERTICAL__;
    color += persistence;
    ...
    color = lerp(float3(luminance, luminance, luminance), color, __SATURATION__);
    color *= float3(__RED_GAIN__, __GREEN_GAIN__, __BLUE_GAIN__);
